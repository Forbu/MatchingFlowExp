...
class FlowTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        with torch.no_grad():
            ...
            # quickly permute the the prior to get a proper OT matching
            distance_matrix = torch.cdist(
                prior.reshape((batch_size, -1)).unsqueeze(0),
                image.reshape((batch_size, -1)).unsqueeze(0),
                p=2.0,
                compute_mode="use_mm_for_euclid_dist_if_necessary",
            )

            a = torch.ones(batch_size).cuda() / batch_size
            b = torch.ones(batch_size).cuda() / batch_size
            
            transport_matrix = ot.emd(a, b, distance_matrix.squeeze(0))

            permutation = torch.argmax(transport_matrix, dim=0)
